...
const frameProps = {  
    ...
    rAccessor: "y",
    oLabel: true,
    margin: { left: 250, bottom: 90, right: 10, top: 40 },
    style: function(e,t){return{fill:"url(#gradient)"}},
    additionalDefs: [
      <linearGradient key="gradient" x1="0" x2="1" y1="0" y2="1" id="gradient">
        <stop stopColor={theme[7]} offset="0%" />
        <stop stopColor={theme[7]} offset="100%" />
      </linearGradient>
    ],
    renderMode:"painty",
    ...
    tooltipContent: d => {
      const bothValues = [
        <div style={{ color: theme[0] }} key={"x"}>
          Product category: {d.x}
        </div>,
        <div style={{ color: theme[1] }} key="y">
          Average amount spent: {d.y}
        </div>
      ]
...
export default class ProductsAvgPayment extends React.Component {
  constructor(props) {
    super(props)
    ...
    return (
        <div>
          <div>
            <MarkdownText
              text={`
              What are people spending more money on? On what type of products?
              What should we try to advertise more given what 'sells' more.`}
            />
